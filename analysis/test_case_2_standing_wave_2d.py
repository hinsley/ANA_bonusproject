...
Forcing: F = (2*pi^2 - 1) * exp(-t) * cos(pi*x) * cos(pi*y)
...
Output:
    analysis/test_case_2_standing_wave_2d_report.txt
    analysis/test_case_2_standing_wave_2d.gif
"""
...
def initial_condition(X: NDArray, Y: NDArray) -> NDArray:
    """Initial condition: u_0 = cos(pi*x) * cos(pi*y)."""
    return exact_solution(X, Y, 0.0)


def forcing_function(X: NDArray, Y: NDArray, t: float) -> NDArray:
    """
    Forcing function: F = (2*pi^2 - 1) * exp(-t) * cos(pi*x) * cos(pi*y).
    """
    coeff = 2 * np.pi**2 - 1
    return (
        coeff * np.exp(-t)
...
def bc_zero_flux(coords: NDArray, t: float) -> NDArray:
...
def main():
    """Run Test Case 2 (2D): Standing Wave."""
    report = Reporter("analysis/test_case_2_standing_wave_2d_report.txt")
    
    report.write("=" * 70)
    report.write("Test Case 2 (2D): The 'Standing Wave'")
    report.write("=" * 70)
    report.write("\nIsotropic diffusion with forcing and homogeneous Neumann BCs.")
    report.write("Exact solution: u = exp(-t) * cos(pi*x)*cos(pi*y)")
    report.write("Forcing: F = (2*pi^2 - 1) * exp(-t) * cos(pi*x)*cos(pi*y)")
    ...
    create_error_plots(
        ...
        save_path="analysis/test_case_2_standing_wave_2d_errors.png",
        title_prefix="Standing Wave (2D)",
    )
    report.write("Error plots saved to: analysis/test_case_2_standing_wave_2d_errors.png")
    ...
    anim = animate_solution_2d(
        ...
        cmap="coolwarm",
        title_prefix="Standing Wave (2D)",
        save_path="analysis/test_case_2_standing_wave_2d.gif",
        interval=100,
    )
    report.write("Animation saved to: analysis/test_case_2_standing_wave_2d.gif")

    report.write("\n" + "=" * 70)
    report.write("Test Case 2 (2D) Complete")
    report.write("=" * 70)
