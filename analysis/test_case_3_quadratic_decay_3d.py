...
Isotropic diffusion with inhomogeneous forcing and Robin boundary conditions.
This tests the 3D Douglas-Gunn solver against an exact analytical solution.

Domain: Unit Cube [0, 1]^3
Exact Solution: u(x,y,z,t) = exp(-t) * (1 + x^2 + y^2 + z^2)
Boundary Conditions: u + du/dn = g(x,y,z,t) (Robin)
Forcing: F = -exp(-t) * (7 + x^2 + y^2 + z^2)
...
Usage:
    python analysis/test_case_3_quadratic_decay_3d.py

Output:
    analysis/test_case_3_quadratic_decay_3d_report.txt
    analysis/test_case_3_quadratic_decay_3d.gif
"""
...
def bc_x_min(coords: tuple, t: float) -> NDArray:
    """Robin BC at x=0: g = exp(-t) * (1 + y^2 + z^2)."""
    Y, Z = coords
    return np.exp(-t) * (1 + Y**2 + Z**2)


def bc_x_max(coords: tuple, t: float) -> NDArray:
    """Robin BC at x=1: g = exp(-t) * (4 + y^2 + z^2)."""
    Y, Z = coords
    return np.exp(-t) * (4 + Y**2 + Z**2)


def bc_y_min(coords: tuple, t: float) -> NDArray:
    """Robin BC at y=0: g = exp(-t) * (1 + x^2 + z^2)."""
    X, Z = coords
    return np.exp(-t) * (1 + X**2 + Z**2)


def bc_y_max(coords: tuple, t: float) -> NDArray:
    """Robin BC at y=1: g = exp(-t) * (4 + x^2 + z^2)."""
    X, Z = coords
    return np.exp(-t) * (4 + X**2 + Z**2)


def bc_z_min(coords: tuple, t: float) -> NDArray:
    """Robin BC at z=0: g = exp(-t) * (1 + x^2 + y^2)."""
    X, Y = coords
    return np.exp(-t) * (1 + X**2 + Y**2)


def bc_z_max(coords: tuple, t: float) -> NDArray:
    """Robin BC at z=1: g = exp(-t) * (4 + x^2 + y^2)."""
    X, Y = coords
    return np.exp(-t) * (4 + X**2 + Y**2)


def create_boundary_conditions():
    """Create Robin BCs: alpha=1, beta=1 => u + du/dn = g."""
    return BoundaryConditions3D(
        x_min=RobinBC(alpha=1.0, beta=1.0, g=bc_x_min),
        x_max=RobinBC(alpha=1.0, beta=1.0, g=bc_x_max),
        y_min=RobinBC(alpha=1.0, beta=1.0, g=bc_y_min),
        y_max=RobinBC(alpha=1.0, beta=1.0, g=bc_y_max),
        z_min=RobinBC(alpha=1.0, beta=1.0, g=bc_z_min),
        z_max=RobinBC(alpha=1.0, beta=1.0, g=bc_z_max),
    )
...
def main():
    """Run Test Case 3 (3D): Quadratic Decay."""
    report = Reporter("analysis/test_case_3_quadratic_decay_3d_report.txt")
    
    report.write("=" * 70)
    report.write("Test Case 3 (3D): The 'Quadratic Decay'")
    report.write("=" * 70)
    report.write("\nIsotropic diffusion with forcing and Robin BCs.")
    report.write("Exact solution: u = exp(-t) * (1 + x^2 + y^2 + z^2)")
    report.write("Forcing: F = -exp(-t) * (7 + x^2 + y^2 + z^2)")
    report.write("Robin BC: u + du/dn = g (alpha=1, beta=1)")
    ...
    create_error_plots(
        ...
        save_path="analysis/test_case_3_quadratic_decay_3d_errors.png",
        title_prefix="Quadratic Decay (3D)",
    )
    report.write("Error plots saved to: analysis/test_case_3_quadratic_decay_3d_errors.png")
    ...
    try:
        output_path = create_volume_animation(
            ...
            output_path="analysis/test_case_3_quadratic_decay_3d.gif",
            cmap="plasma",
            opacity=0.95,
            title_prefix="Quadratic Decay (3D)",
            fps=15,
    ...
    report.write("\n" + "=" * 70)
    report.write("Test Case 3 (3D) Complete")
    report.write("=" * 70)
