...
Exact Solution: u(x,y,z,t) = exp(-3*pi^2*t) * sin(pi*x) * sin(pi*y) * sin(pi*z)
...
Usage:
    python analysis/test_case_1_decaying_bubble_3d.py

Output:
    analysis/test_case_1_decaying_bubble_3d_report.txt
    analysis/test_case_1_decaying_bubble_3d.gif
"""
...
def initial_condition(X: NDArray, Y: NDArray, Z: NDArray) -> NDArray:
    """Initial condition: u_0 = sin(pi*x) * sin(pi*y) * sin(pi*z)."""
...
def bc_zero(coords: tuple, t: float) -> NDArray:
...
def run_solver(nx: int, dt: float, t_final: float = 0.1) -> tuple:
    """
    Run the solver with given grid resolution and time step.

    Returns (times, solutions, X, Y, Z, errors_linf, errors_l2, dx).
    """
    ...
    bc = BoundaryConditions3D(
        ...
        z_min=DirichletBC(bc_zero), z_max=DirichletBC(bc_zero),
    )
...
def run_convergence_study(report: Reporter) -> list:
    ...
    t_final = 0.02  # Short time to reduce temporal error contribution.
...
def main():
    """Run Test Case 1 (3D): Decaying Bubble."""
    report = Reporter("analysis/test_case_1_decaying_bubble_3d_report.txt")
    
    report.write("=" * 70)
    report.write("Test Case 1 (3D): The 'Decaying Bubble'")
    report.write("=" * 70)
    report.write("\nIsotropic diffusion with homogeneous Dirichlet BCs.")
    report.write("Exact solution: u = exp(-3*pi^2*t) * sin(pi*x)*sin(pi*y)*sin(pi*z)")
    report.write("Time range: t in [0, 0.1]")
    ...
    dt = 0.0005
    t_final = 0.1
    report_every = 2  # Interval for text output (matching original save_every).
    ...
    create_error_plots(
        ...
        save_path="analysis/test_case_1_decaying_bubble_3d_errors.png",
        title_prefix="Decaying Bubble (3D)",
    )
    report.write("Error plots saved to: analysis/test_case_1_decaying_bubble_3d_errors.png")
    ...
    try:
        output_path = create_volume_animation(
            ...
            output_path="analysis/test_case_1_decaying_bubble_3d.gif",
            cmap="inferno",
            opacity=0.95,
            title_prefix="Decaying Bubble (3D)",
            fps=15,
    ...
    report.write("\n" + "=" * 70)
    report.write("Test Case 1 (3D) Complete")
    report.write("=" * 70)
